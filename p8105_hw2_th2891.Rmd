...
There are `r nrow(trash_df)` observations in `trash_df` and the variables are `r names(trash_df)`. There are `nrows(precip_df)` observations in `precip_df`, which combines the precipitation data from 2018 and 2019. The varaibles in this combined data set are `r names(precip_df)`.

The median number of sports balls in a dumpster in 2019 was `r median(trash_df$sports_balls, na.rm=TRUE)`. 
...
There are `r nrow(combined_df)` observations in `combined_df` and the variables are `r names(combined_df)`. 
* range of years
* 
...
Making a scatter plot fro white non-hispanic children born in 2016
